            ...
            <AnimatePresence>
              {testimonials.map((testimonial, index) => (
                <motion.div
                  ...
                  onMouseEnter={() =>
                    isActive(index) && setIsHoveringActive(true)
                  }
                  onMouseLeave={() =>
                    isActive(index) && setIsHoveringActive(false)
                  }
                  onTouchStart={() => {
                    if (isActive(index)) {
                      setIsHoveringActive(true);
                      // На мобилках цветная при любом касании
                      setTimeout(() => setIsHoveringActive(false), 3000);
                    }
                  }}
                >
                  <div className="relative h-full w-full">
                    <Image
                      ...
                      className={`h-full w-full rounded-3xl object-cover object-center transition-all duration-300 ${
                        ...
                        // Неактивные картинки всегда серые
                        isActive(index) && isHoveringActive
                          ? "grayscale-0"
                          : "grayscale"
                      }`}
                    />
                    {/* Наложение для лучшей видимости на серых изображениях */}
                    {!(isActive(index) && isHoveringActive) && (
                      <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-transparent to-black/10 mix-blend-overlay" />
                    )}
                  </div>
                </motion.div>
              ))}
